...
export async function signUp(name: string, email: string, password: string): Promise<UserDTO> {
  const { data } = await http.post<AuthResponse>('/auth/signup', { name, email, password });
  saveToken(data.token);
  return data.user;
}
